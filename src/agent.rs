/// Test agent comment
use std::{cell::RefCell, rc::Rc, sync::{atomic::AtomicBool, mpsc::channel, Arc}, thread::{self, JoinHandle}};
use crate::{permit_store::PermitStore, state::{self, AgentStateMachine, MoveAgentStateMachine, State, StateChanged}};

// Move agents have non-thread-safe state information
#[derive(Debug)]
pub struct MoveAgent<Data: Send + Sync + 'static> {
    // Guaranteed data
    start_state: Arc<dyn State<Data>>, // State is owned (on the heap)
    data_store: Arc<Data>,
...
impl<Data: Send + Sync> MoveAgent<Data> {
    ...
    pub fn run_move(&mut self, state: Arc<(dyn State<Data> + 'static)>) {
        ...
        //     let mut masm = MoveAgentStateMachine::new(state);
        ...
        //     //     let mut next_state: Option<Rc<RefCell<dyn State<Data>>>> = MoveAgentStateMachine::advance_state_move(state,&data);
        ...
        //     //         // Advance the state
        //     //         next_state = MoveAgentStateMachine::advance_state_move(state,&data);
...
pub struct CloneAgent<Data: Send + Sync + 'static> {
    // Guaranteed data
