...
use crate::{agent::{AgentRef, AgentRunStatus, AgentThreadStatus, CloneAgent, MoveAgent}, state::{State, StateChanged}};
...
pub struct MoveAgentProgram<Config, Data: Send + Sync + 'static> {
    agent: Arc<MoveAgent<Data>>,
    agent_ref: AgentRef<Data>,
    _phantom: PhantomData<Config>
}

impl<Config, Data: Send + Sync + 'static + FromConfigMove<Config>> MoveAgentProgram<Config, Data> {
    ...
    pub fn start(config: &Config)->Result<Self, AgentProgramError<Self>> {
        ...
        let mut agent = MoveAgent::new(start_state.clone(), Data::from_config(config));
        agent.run_move(start_state);
        let agent_ref = agent.get_ref();
        match agent_ref {
            Some(agent_ref) => {
                Ok(MoveAgentProgram { agent: Arc::new(agent), agent_ref, _phantom: PhantomData })
            },
    ...
    pub fn complete(self)->Result<Arc<Data>, AgentProgramError<Self>>{
        ...
        //  However, it could be possible that a weak reference could be upgraded in the time between the previous check, and the unwrap below
        ...
        match Arc::try_unwrap(self.agent) {
            ...
            Err(old_arc) => {
                ...
                let ap = MoveAgentProgram { agent: old_arc, agent_ref: self.agent_ref, _phantom: PhantomData::<Config> };
